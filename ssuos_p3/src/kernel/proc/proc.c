...
void init_proc()
{
	...
	cur_process->console = get_console();
	//foreground프로세스와 콘솔을 현재 프로세스로 바꿔줌
	cur_foreground_process = cur_process;
	cur_console = cur_process->console;
	
	list_push_back(&f_list, &cur_process->elem_foreground);//f_list에 넣음
...
pid_t proc_create(proc_func func, struct proc_option *opt, void* aux)
{
	...
	*(--top) = 6; //edi
	...
	if(opt!=NULL&&opt->foreground)//foreground 옵션일 때
	{
