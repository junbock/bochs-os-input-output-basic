...
void clearScreen(void)
{
	...
	while(1)
	{
		...
		tmp_buf_p = (char*)((int)cur_console->buf_p + (HSCREEN * 1));
		tmp_buf_w = cur_console->buf_w + SIZE_SCREEN;
		if(tmp_buf_w > SCROLL_END)
			tmp_buf_w = (char *)((int)tmp_buf_w - SIZE_SCROLL);
		//glob_y가 0이 될때까지 scroll 호출하여 콘솔클리어 구현
		if(tmp_buf_p > cur_console->buf_w && cur_console->buf_p <= cur_console->buf_w)
			if(cur_console->Glob_y == 0)
				break;
			else
				scroll();
...
#endif
	//호출한 프로세스의 콘솔이 현재 사용하는 콘솔일 때까지 기다림 
	while(1)
	{
		
		if(cur_process->console == cur_console)
		{
...
void scroll_screen(int offset)
{
	...
	if(cur_console->sum_y < NSCROLL && offset < 0 && tmp_buf_p <= cur_console->buf_s && cur_console->buf_p > cur_console->buf_s) return;// 처음
